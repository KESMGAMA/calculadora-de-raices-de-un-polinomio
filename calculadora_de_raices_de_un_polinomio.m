...
if(grado >= 3)
    ...
    sin_dos(dist,delta_r,delta_s,iter,grado,raices,raices_encontradas,raices_imaginarias,raices_imaginarias_encontradas,grado_nuevo,a,r,s,exacto,termino_independiente,ecuacion_original); %parte del código que utiliza el método de Bairstow
else
    if(grado == 2)
...
function sin_dos(dist,delta_r,delta_s,iter,grado,raices,raices_encontradas,raices_imaginarias,raices_imaginarias_encontradas,grado_nuevo,a,r,s,exacto,termino_independiente,ecuacion_original)
    b = [];
    c = [];
    if(grado == 0)
        disp('////////////////////////////');
        ...
        while(raices_encontradas + raices_imaginarias_encontradas ~= grado) %método de Bairstow
            if(grado_nuevo <= 2)
                ...
                while(iter == 0 || delta_r > dist || delta_r < -dist || delta_s > dist || delta_s < -dist)
